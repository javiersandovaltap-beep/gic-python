...
def test_persistencia():
    """
    Función de prueba para verificar la persistencia JSON.
    Ejecutar: python utils/persistencia.py
    """
    print("=" * 70)
    print("PRUEBA DE PERSISTENCIA JSON - GIC")
    print("=" * 70)
    
    # Crear instancia con archivo de prueba
    ...
    try:
        ...
        for cliente in clientes_cargados:
            print(f"  • {cliente['nombre']} ({cliente['tipo_cliente']})")
    except Exception as e:
    ...
    try:
        ...
        for respaldo in respaldos[:3]:  # Mostrar solo los 3 más recientes
            fecha = respaldo['fecha'].strftime('%Y-%m-%d %H:%M:%S')
            print(f"  • {respaldo['nombre']} - {fecha}")
    except Exception as e:
        print(f"❌ Error al listar: {e}")
    
    print("\n5️⃣  ESTADÍSTICAS DEL ARCHIVO")
    print("-" * 70)
    try:
    ...
    print("\n6️⃣  VALIDACIÓN DE ARCHIVO")
    print("-" * 70)
    try:
        es_valido = persistencia.validar_archivo()
        print(f"{'✓' if es_valido else '❌'} Archivo válido: {es_valido}")
    except Exception as e:
